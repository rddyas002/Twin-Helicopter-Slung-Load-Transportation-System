...
TCP_client::TCP_client(int id, int port, const char * ipaddress, OpencvCamera * vCam) {
    ...
    char file_buffer[30];
    sprintf(file_buffer,"image_data_client%d.txt", client_id);
...
void TCP_client::write2server(const char * data){
    ...
    else
    {
        ...
        if (n < 0) 
             errorMessage("Writing to socket");
//        if (n == strlen(data)){
//            //std::cout << "Data packet sent to server." << std::endl;
//        }
    }
}
...
void TCP_client::receiveThread(void){
    debugMessage("Into receive thread");
    int read_zero = 0;
    ...
    while(!quit_rx_thread)
    {
        ...
        {    
            ...
            {    
//                data_received(&rx_buffer[0]);        
                TCP_client::loadData(&rx_buffer[0]);
...
//        }
    ...
    debugMessage("Out of receive thread");  
...
void TCP_client::decodeData(void){
