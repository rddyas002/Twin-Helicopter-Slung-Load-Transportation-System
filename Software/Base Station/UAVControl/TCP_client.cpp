...
TCP_client::TCP_client(int id, int port, const char * ipaddress, OpencvCamera * vCam, double start_time) {
...
unsigned int TCP_client::timeNow(void){
    struct timespec now;
...
//    CPU_SET(2, &my_set);
    ...
    for (int i = 0; i < 4; i++){
        if (CPU_ISSET(i, &my_set))
            printf("Core [%d] is used for tid [%lu]\r\n", i, pthread_self());
    }
...
void TCP_client::loadData(char * data){
    ...
    int len = sprintf(log_buffer,"%s",data);
    sprintf(&log_buffer[len-3],",%u#\r\n",timeNow());
    writeLog();         
